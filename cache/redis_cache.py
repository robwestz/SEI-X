...
class RedisCache(CacheBackend):
    ...
    async def incr(self, key: str) -> Optional[int]:
        ...
        try:
            ...
            logging.error(f"Cache incr error: {e}")
            return None
    ...
    async def expire(self, key: str, ttl: int) -> bool:
        ...
        try:
            ...
            logging.error(f"Cache expire error: {e}")
            return False
...
class MemcachedCache(CacheBackend):
    ...
    async def incr(self, key: str) -> Optional[int]:
        """Memcached does not support counter operations in this implementation."""
        return None

    async def expire(self, key: str, ttl: int) -> bool:
        """Memcached does not support expire in this implementation."""
        return False
...
class FallbackCache(CacheBackend):
    ...
    async def incr(self, key: str) -> Optional[int]:
        """Increment a counter in the active backend if supported."""
        if not self.active_backend:
            return None

        incr = getattr(self.active_backend, "incr", None)
        ...
        try:
            ...
            logger.error(f"Fallback cache incr failed: {e}")
            return None

    async def expire(self, key: str, ttl: int) -> bool:
        """Expire a key in the active backend if supported."""
        if not self.active_backend:
            return False

        expire = getattr(self.active_backend, "expire", None)
        ...
        try:
            ...
            logger.error(f"Fallback cache expire failed: {e}")
            return False
